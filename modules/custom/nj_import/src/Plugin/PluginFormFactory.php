...
class PluginFormFactory {
   ...
   *
   * @param \Drupal\nj_imports\Plugin\Type\ImportsPluginInterface $plugin
   *   The Imports plugin.
   * @param string $operation
   *   The type of form to check for, which can be for example:
   *   - configuration
   *     Displayed when configuring the import type.
   *   - import
   *     Displayed on the import add/edit form.
   *   - option
   ...
   */
  public function hasForm(ImportsPluginInterface $plugin, $operation) {
   ...
   */
  public function createInstance(ImportsPluginInterface $plugin, $operation) {
    $definition = $plugin->getPluginDefinition();
